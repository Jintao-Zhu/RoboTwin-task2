...
class RLASState:
    ...
    def update(self, current_losses: np.ndarray, step: int):
        ...
        logits = positive / self.temperature
    ...
    def stats(self):
        ...
        return {
            ...
            "reducible_mean": None if self.last_reducible is None else float(self.last_reducible.mean()),
            "reducible_std": None if self.last_reducible is None else float(self.last_reducible.std()),
            "positive_ratio": None if self.last_positive is None else float((self.last_positive > 0).mean()),
            "positive_max": None if self.last_positive is None else float(self.last_positive.max()),
